...
- **依頼箱（box）**: `~/.herdr-taskforce/box.db`（sqlite）。ツール
  ...
  - **box は「ユーザー⇔チーム境界（インターフェース⇔リーダー）」専用**。
    ビルダーズ/エバリュエータズは box を一切使わない（送受信とも禁止）。
...
4. タスクの数・難易度・並列可能数に応じて、**ビルダーズを別タブに起動**
   ...
   （role="builder"）を使い、モデルは conf.json に任せる。各ビルダーズへの
   命令文には **「自分のロール規定（`~/.herdr-taskforce/docs/roles.md` の
   該当節）を読むこと。完了報告は herdr 経由（box は使わない）」** を明示し、
   `htf_agent_prompt(agent=<builder名>, wait=true)` で送る。
...
3. 完了したら **herdr 経由で**リーダーに報告（タスク id + 変更概要 + 検証方法）。
   box は使わない。
...
3. 完了したら **herdr 経由で**リーダーに報告（指摘一覧 + パス）。box は使わない。
